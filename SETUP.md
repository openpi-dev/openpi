...
- `/plan [objective]` explores read-only before changing anything: while armed it blocks `edit`, `write`, `bash`, and the delegation tools (`subagent_spawn`, `subagent_send`, `workflow`, `bg_start`), while read/grep/find/ls/fd/rg stay available. `/plan done` asks for approval and then releases the gate; `/plan off` cancels. It gates this interactive session only — a headless subagent or workflow child runs in its own session, which is why spawning them is blocked rather than gated.

# 稳定 ID 还不够：Agent 任务台账如何避免变成第二个 Workflow
...
所以我们要做一份 **Session Task Ledger**。Todo UI 漂不漂亮无关紧要；它要记住“还有什么工作”，但不能接管“工作怎么执行”。
...
子 agent 返回成功，只说明它正常结束了。它可能漏读文件，可能只写了一半，也可能测试根本没跑。父 agent 还没看结果，Task 已经显示完成，这张台账就从“提醒”变成了“制造错误信心”。

Maka 的 [Session Task Ledger 文档](https://github.com/maka-agent/maka-agent/blob/f9e78d17e1ae42e3428390baf9d458cf4f30a00b/docs/session-task-ledger-lifecycle.md) 给了我们一个很重要的约束：**子 agent 可以认领 Task，但没有完成权。** 即使子 agent 成功返回，父 agent 也要审查结果，再写入完成证据。
...
```ts
pi.on("context", (event) => ({
  messages: [...event.messages, buildLedgerContextMessage(snapshot)],
}));
```

投影放在消息尾部，不去改前面的共同前缀。它最大的好处是用完即弃，不会把旧状态留到下一轮。预算也从 4000 字符砍到 800。没有未完成 Task 时，一个字都不注入。

这一步不高级，但少了它，Task Ledger 会自己污染 Context。
...
GPT-5.6 Sol 还指出，advisory ledger 里的 `failed` 很含糊：失败的是任务，还是某次尝试？如果工作仍然要做，它应该回到 `pending`；如果缺条件，它应该是 `blocked`；如果决定不做，叫 `dropped` 更准确。
...
没有 Owner，没有 Deadline，没有 Priority，没有依赖图。那些字段以后可能有用。v1 每加一个，Ledger 都更像调度器。

字段还有几条硬限制：`subject` 最多 120 字符，`detail` 和 `note` 各 500；离开 `blocked / done / dropped` 时清掉旧 note，除非这次更新同时写入新 note。三个 `ledger_*` 工具只能由父 Session 使用，Pi Subagent 和 Workflow 子会话统一从共享 denylist 排除。
...
不能遇到坏快照就悄悄退回上一版。上一版的 `nextId` 可能更小，后续重新分配 T3，同一条分支里就出现两个意思不同的 T3。宁可显示 Ledger 恢复失败，也不要把旧状态装成新状态。

## 为什么 v1 不接 Footer，也不接 Subagent

我们刚给 Pi Footer 做完一轮配置，里面有一条原则：Subagent、Workflow、后台终端属于运行状态，必须始终可见。

未完成的 Ledger Item 只是一份意图记录。它可能放在那里几小时，也可能只是用户暂时不做的提醒。把 `tasks 7 open` 常驻 Footer，会把意图伪装成正在运行的工作，还会一直占一行。
...
Subagent 联动也不做。后面真要加 `ledger_id`，边界仍然是：Spawn 成功可以记录关联，子 agent 成功不能自动写 `done`。父 agent 看完结果，才能把证据写进去。

我们还发现一个更眼前的问题：本机已经安装了 `~/.pi/agent/extensions/todo.ts`。Todo 和 Ledger 同时暴露给模型，工具选择一定会乱。Ledger 上线前，这个旧工具必须停用；否则宁可不注册新工具。

## 先设删除条件，再写代码

Task Ledger 很容易越做越像项目管理软件。我们给 v1 留了一个不太体面的出口：用十个长 Session 做 dogfood，如果没有证明价值，就删掉。
...
Ledger Entry 占 Session JSONL    < 5%

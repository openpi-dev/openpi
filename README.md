...
输入 `/` 可查看原生发现的命令及不可用原因。`/openpi-setup` 使用原生配置回合；无参数的 `/ps`、`/lg`、`/subagents`、`/usage`、`/btw` 打开对应 Web 面板，不发送模型请求。这些面板快捷命令不接收参数或图片，带参时保留输入并提示。运行中的回合显示“已处理”时间，结束后显示“用时”；时间从 Pi 实际开始执行计算，并随原生 Session 保存。没有可靠耗时记录的旧回合不推算补值。
...
长会话可向上加载更早的消息，每页保留 250 条/2 MiB 上限，阅读窗口上限为 1000 条/8 MiB。加载和流式追加保持阅读位置；发送新消息或点击“跳至最新”返回当前尾部。单条超长消息会明确标注截断并保留其原生记录身份。运行中有草稿时同时提供发送和停止按钮，排队数来自 Pi；已接收不表示已经执行完成。

变更面板默认展示 Git 未暂存变更，也可切换到暂存、分支或会话基线。列表与逐文件 diff 分开加载，大型未跟踪文件不会阻止读取整个工作区；达到显示限额会标注截断。会话基线仍是有容量限制的附加视图，不能替代 Git。跨 worktree 的原生编辑记录可以在“生成文件”中查看；读取工作区外的文件需点击“只读打开此文件”，仅授权该文件，不放宽整个目录。

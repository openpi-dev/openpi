...
The root `tsconfig.json` owns the Effect language-service plugin and the shared strict
compiler options. An extension keeps a small `tsconfig.json` only to define its local
project scope:

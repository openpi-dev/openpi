...
export function buildInteractiveSetupPrompt(options: {
  ...
  return [
    ...
    "- Next-action suggestions: disabled, or model-generated after a fully settled main-agent run. A suggestion appears on a dedicated dim row inside an empty editor so CJK IME preedit cannot overlap it; Right accepts it without submitting, and any other editor input dismisses it. Enabling requires an available provider/model and reasoning level and adds one small model call per settled run.",

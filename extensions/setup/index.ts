      ...
      ui_footer_lines: Type.Optional(
        Type.Array(
          Type.Array(StringEnum(FOOTER_LAYOUT_ITEMS), { minItems: 1 }),
          {
            minItems: 1,
            description:
              "Declarative multi-line footer layout. Each row is an ordered list of metrics (cwd, cwd-base, model, thinking, context, cache, cost, throughput, git, pr) plus at most one flex for left/right alignment. cwd shows the full path with home abbreviated to ~; cwd-base shows only the last path segment, which keeps a deep repository path from crowding out the rest of the row. Unknown/duplicate metrics are dropped; empty result falls back to the default. Cannot be combined with ui_footer_items.",
          },
        ),
      ),

...
New calls must use `wait`. During the current deprecation window, legacy `background` remains accepted and returns an actionable warning on every use: replace `background: true` with `wait: false`, or `background: false` with `wait: true`. The alias will be removed in the next breaking release; persisted artifact/details fields named `background` remain runtime facts and are not part of that removal.

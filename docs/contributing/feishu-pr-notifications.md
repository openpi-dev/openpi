...
The Feishu group bot notification is triggered by `.github/workflows/feishu-pr-notification.yml`. The caller pins the reusable implementation in [`openpi-dev/automation`](https://github.com/openpi-dev/automation) to an immutable commit.
...
The workflow runs when a pull request is opened or marked ready for review. It uses `pull_request_target` so the repository secret is available for PRs from forks. The repository caller passes only the two Feishu secrets to the pinned reusable workflow; neither workflow checks out or executes pull request code.
